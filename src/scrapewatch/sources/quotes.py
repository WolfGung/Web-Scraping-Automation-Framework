...
class QuotesSource:
    ...
    with the HTTP sources, so `stats` only ever reports what `client` separately saw
    (robots.txt, and, if `cross_check()` ran, the API call) alongside the pages and
    records this source counted itself.
    """
    ...
    def fetch(self) -> Iterator[RawRecord]:
        ...
        if not self._client.allowed(root_url):
            ...
            raise RuntimeError(f"quotes: fetch refused by robots.txt: {reason}")

        page_url = f"{self._base_url}/js/"
        pages_fetched = 0
        while True:
            html = self._session.render(page_url, wait_for=".quote")
            pages_fetched += 1
    ...
    def cross_check(self) -> int:
        ...
        """
        html = self._session.render(f"{self._base_url}/js/", wait_for=".quote")
